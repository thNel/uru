...
import {defaultRoulettePointerSrc} from '../common/images';
...
import {
  ...
  DEFAULT_OUTER_BORDER_WIDTH, DEFAULT_OUTER_RADIUS,
  DEFAULT_RADIUS_LINE_COLOR,
  DEFAULT_RADIUS_LINE_WIDTH,
  DEFAULT_SPIN_DURATION,
  DEFAULT_TEXT_COLORS,
  DEFAULT_TEXT_DISTANCE, WEB_FONTS,
} from '../../strings';
...
interface Props extends HTMLAttributes<HTMLDivElement> {
  ...
  rotationContainerStyle?: CSSProperties,
}
    ...
    if (mustStartSpinning && !isCurrentlySpinning) {
      ...
      const selectedPrize =
        prizeMap[prizeNumber][
          Math.floor(Math.random() * (prizeMap[prizeNumber]?.length ?? 1))
          ];
    ...
    <RouletteContainer
      ...
      style={
        ...
        (totalImages > 0 && loadedImagesCounter !== totalImages)
          ? {...props.style, visibility: 'hidden', maxWidth: outerRadius, maxHeight: outerRadius}
          : {...props.style, maxWidth: outerRadius, maxHeight: outerRadius}
      }
    >
      <RotationContainer
        className={getRouletteClass() + " rounded-circle"}
        classKey={classKey}
      ...
      <RoulettePointerImage
        ...
        src={pointerProps?.src ?? defaultRoulettePointerSrc}
        alt="roulette-static"
      />
    </RouletteContainer>
  );
